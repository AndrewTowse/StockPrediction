...
def createDataSetInit(csv, timePeriod):
    ...
    #removes the first 500 rows of data from the data frame
    data = data.iloc[100:]
    ...
    #creates a normaliser to be used in later functions upon return
    dataNormalised = normalise.fit_transform(data)
    '''

    #############Makes the data Normaliser ################
    normalise = preprocessing.MinMaxScaler()
    normalise = normalise.fit(data)
    joblib.dump(normalise, 'dataNormaliser.pkl')
    #######################################################

    #normaliser = joblib.load('dataNormaliser.pkl')
    dataNormalised = normalise.transform(data)
    ...
    lastClose = np.array([data[:, 3][i + dayRange].copy() for i in range(len(data) - dayRange)])
    lastClose = np.expand_dims(lastClose, -1)
    ...
    openNormaliser = preprocessing.MinMaxScaler()
    openNormaliser = openNormaliser.fit(nextOpen)
    joblib.dump(openNormaliser, 'openNormaliser.pkl')
    #######################################################

    openNormaliser = joblib.load('openNormaliser.pkl')
    ...
    return ohlcArrayNorm, smaArrayNorm, macdArrayNorm, emaArrayNorm, nextOpenNorm, nextOpen, openNormaliser, lastClose



#this the same method as above except it will load
# the normalisers the network was trained with instead of creating them
def createDataSet(csv, timePeriod):
    ...
    dataNormalised = normalise.fit_transform(data)
    '''
    #loads the normaliser for the old larger dataset
    normaliser = joblib.load('dataNormaliser.pkl')
    dataNormalised = normaliser.transform(data)
    ...
    #loads the nextOpen normaliser for the entire dataset
    openNormaliser = joblib.load('openNormaliser.pkl')
